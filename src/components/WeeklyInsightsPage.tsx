...
const moodDistribution = [
  { name: "행복", value: 35, color: "#FFF4B3" },
  { name: "평온", value: 30, color: "#C4E5D4" },
  { name: "설렘", value: 20, color: "#FFE0A3" },
...
export function WeeklyInsightsPage() {
  return (
    <div className="min-h-screen pb-24" style={{ maxWidth: "390px", margin: "0 auto" }}>
      <div className="p-6 space-y-5">
        ...
        <Card className="p-6 rounded-2xl border shadow-soft-lg">
          <h4 className="mb-4 text-accent-foreground font-medium">자주 등장한 주제</h4>
          <div className="space-y-3.5">
            {keywords.map((keyword, index) => (
              <div key={index}>
                <div className="flex justify-between items-center mb-2">
                  <span className="text-sm font-medium text-foreground">{keyword.word}</span>
                  <span className="text-xs text-muted-foreground">{keyword.count}회</span>
                </div>
        ...
        <Card className="p-6 rounded-2xl border bg-primary/10 shadow-soft-lg">
          ...
          <div className="bg-card rounded-xl p-5 border">
            <p className="text-sm text-foreground/90 leading-relaxed">
              이번 주는 대부분 행복하고 평온한 감정을 느끼셨어요. 특히 사회적 관계가 당신의 행복에 큰 역할을 하고 있네요. 
              감정의 균형이 좋아지고 있고, 작은 순간들에서 기쁨을 찾고 계세요. 
              우정을 계속 가꾸어가세요—분명 긍정적인 에너지의 원천이 되고 있어요! 💚
            </p>
          </div>
        </Card>
        ...
        <Card className="p-5 rounded-2xl border bg-secondary/10 shadow-soft-lg">
          <h4 className="mb-4 text-secondary-foreground font-medium text-center">주간 요약</h4>
