...
export function ProfileModal({ open, onClose, nickname, onLogout }: ProfileModalProps) {
  const [editedNickname, setEditedNickname] = useState(nickname);
  const [timeOfDay, setTimeOfDay] = useState<"am" | "pm">("pm");
  const [notificationHour, setNotificationHour] = useState("9");
  const [notificationMinute, setNotificationMinute] = useState("00");
  const [feedbackStyle, setFeedbackStyle] = useState<"comfort" | "advice">("comfort");
